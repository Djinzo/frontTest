...
async function editProduct(currentEditingProductId, newTitle) {
  if (newTitle) {
    try {
      ...
      if (response) {
        ...
        rows.forEach(row => {
          if (row !== productTable.tBodies[0].rows[0]) { // Assuming the first row is the header
            row.remove();
          }
        });
        addProduct();
  ...
  getProducts().then(products => {
    ...
    products.forEach(product => {
      ...
      editButton.onclick = () => {
        ...
        <p><input tupe="text" value="${product.title}"></p>
        <button onclick="editProduct('${product.id}','${product.title}')">Confirm</button>
...
document.getElementById("confirmButton").addEventListener("click", addProductFromInput);
